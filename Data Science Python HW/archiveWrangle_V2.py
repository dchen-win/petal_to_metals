...
parser.add_argument("-z", "--zipfile", type=str, help="Name of 7z file to decompress",
                    required=True)
...
# 'superuser.com.7z'  # For mid-term project
...
def get_comments(path):
    ...
    i = 0
    # TODO write JSON from Dict
    for _, elem in iterparse(f"{path}/Comments.xml", events=("end",)):
...
def get_data(path):
    ...
    post_df.drop('Body', inplace = True, axis=1)
    post_df.drop('OwnerDisplayName', inplace = True, axis=1)
    post_df.drop('FavoriteCount', inplace = True, axis=1)
    post_df.drop('ClosedDate', inplace = True, axis=1)
    post_df.drop('LastEditorDisplayName', inplace = True, axis=1)
    post_df.drop('CommunityOwnedDate', inplace = True, axis=1)
    print(f"post_df {post_cols}")
    print(f"post_df\n{post_df.dtypes}")
    print(f"{post_df.head(5)}")
    post_df.to_csv(f"{extract_path}/posts.csv", sep=",", index=False)
    ...
    #fill empty with 0 and drop UserDisplayName column
    posthistory_df.fillna(0, inplace=True)
    posthistory_df = posthistory_df[posthistory_df['UserId'] != 0]
    posthistory_df.drop('UserDisplayName', inplace = True, axis=1)
    posthistory_df.drop('Text', inplace = True, axis=1)
    print(f"posthistory_df {posthistory_df.columns.tolist()}")
    print(f"{posthistory_df.head(5)}")
    posthistory_df.to_csv(f"{extract_path}/posthistory.csv", sep=",", index=False)
    ...
    users_df.drop('AboutMe', inplace = True, axis=1)
    users_df.drop('ProfileImageUrl', inplace = True, axis=1)
    users_df.drop('WebsiteUrl', inplace = True, axis=1)
    print(f"{users_df.head(5)}")
    users_df.to_csv(f"{extract_path}/users.csv", sep=",", index=False)
    ...
    votes_df.drop('UserId', inplace = True, axis=1)
    votes_df.drop('BountyAmount', inplace = True, axis=1)
    ...
    votes_df.to_csv(f"{extract_path}/votes.csv", sep=",", index=False)
    ...
    tags_df.to_csv(f"{extract_path}/tags.csv", sep=",", index=False)
    ...
    postLinks_df.to_csv(f"{extract_path}/postLinks.csv", sep=",", index=False)
    ...
    badges_df.to_csv(f"{extract_path}/badges.csv", sep=",", index=False)
    ...
    comments_df = comments_df[comments_df['UserId'] != 0]
    comments_df.drop('UserDisplayName', inplace = True, axis=1)
    print(f"comments_df {comments_df.dtypes}")
    print(f"comments_df {comments_df.columns.tolist()}")
    print(f"{comments_df.head(5)}")
    comments_df.to_csv(f"{extract_path}/comments.csv", sep=",", index=False)
